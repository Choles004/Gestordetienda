document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadInitialConfigAndData() {
        ...
        const businessType = loadData('businessType');
        const isRestaurant = businessType === 'restaurant';

        const restaurantLink = document.getElementById('restaurant-link');
        if (restaurantLink) restaurantLink.classList.toggle('hidden', !isRestaurant);

        if (!isRestaurant) {
            console.warn("Business type is not 'restaurant'. Hiding restaurant features.");
            const layout = document.querySelector('.restaurant-layout');
            if (layout) layout.innerHTML = '<p class="report-message" style="margin:20px;"><i class="fas fa-exclamation-triangle"></i> Esta función es solo para tipo de negocio "Restaurante". Cambia el tipo en Configuración.</p>';
            manageMenuButton.style.display = 'none';
            manageTablesButton.style.display = 'none';
            manageDeliveryButton.style.display = 'none';
            return; // Stop further loading for non-restaurants
        }
    ...
    function renderOrderItems(items) {
        ...
        items.forEach(item => {
            ...
            nameSpan.title = item.name || 'Producto Desconocido'; // Tooltip for long names
            ...
            removeBtn.onclick = () => removeOrderItem(li.dataset.productId); // Use productId for removal
